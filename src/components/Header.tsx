...
export const Header: React.FC<HeaderProps> = ({
  ...
  const navLinks: { id: 'home' | 'catalog' | 'club' | 'guides'; label: string }[] = [
    ...
    { id: 'catalog', label: 'Los 3 Orígenes' },
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-6 sm:px-10 lg:px-12 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8 lg:gap-12">
          {navLinks.map((link) => (
            <button
              ...
              className={`text-[11px] uppercase tracking-[0.2em] font-medium transition-colors cursor-pointer py-1 ${
                activeTab === link.id
                  ? 'text-[#f5eedf] border-b border-[#d49a55]'
                  : 'text-[#8e857c] hover:text-[#f5eedf]'
              }`}
            >
              {link.label}
            </button>
        ...
        <div className="flex items-center gap-3 sm:gap-5">
          ...
          <button
            ...
            className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full border transition-all cursor-pointer text-xs ${
              activeTab === 'club'
                ? 'bg-white/10 border-[#d49a55] text-white'
                : 'bg-white/[0.02] border-white/10 text-[#a19688] hover:text-white hover:border-white/20'
            }`}
          >
            <User className="w-3.5 h-3.5 text-[#d49a55]" />
            <span className="hidden sm:inline font-medium">{loyaltyProfile.customerName.split(' ')[0]}</span>
            <span className="px-1.5 py-0.2 rounded-full text-[9px] bg-white/5 font-semibold text-[#d49a55]">
              {loyaltyProfile.tier}
            </span>
          </button>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden fixed inset-x-0 top-[56px] bg-black/95 border-b border-white/10 px-6 py-6 space-y-4 backdrop-blur-xl shadow-2xl animate-in fade-in duration-200">
          ...
          <div className="pt-3 border-t border-white/10">
            ...
            >
              ...
              <span>Club Privé & Membresía ({loyaltyProfile.customerName.split(' ')[0]})</span>
            </button>
          </div>
        </div>
      )}
