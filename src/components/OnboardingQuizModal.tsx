...
export const OnboardingQuizModal: React.FC<OnboardingQuizModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-black/85 backdrop-blur-xl animate-in fade-in duration-300">
      ...
      <div className="relative w-full max-w-2xl max-h-[92vh] sm:max-h-[90vh] flex flex-col rounded-3xl bg-[#0a0a0a] border border-white/10 p-4 sm:p-8 shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between pb-3 sm:pb-4 border-b border-white/5 shrink-0">
          ...
          <button
            onClick={onClose}
            className="p-1.5 sm:p-2 rounded-full text-[#8c8276] hover:text-white hover:bg-white/5 transition-colors cursor-pointer"
        ...
        {/* Scrollable Body */}
        <div className="flex-1 overflow-y-auto overscroll-contain pr-1 py-3 sm:py-5">
          {!resultBean ? (
            ...
            <div className="space-y-4 sm:space-y-6">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-[11px] sm:text-xs text-[#8c8276]">
              ...
              <div className="space-y-1 sm:space-y-1.5">
                <h3 className="text-base sm:text-2xl font-bold text-[#f7eedf] tracking-tight leading-snug">
                  {questions[currentStep].question}
                </h3>
                <p className="text-[11px] sm:text-xs text-[#9e9386]">
              ...
              <div className="space-y-2 sm:space-y-3 pt-1">
                {questions[currentStep].options.map((option, idx) => (
                  <button
                    ...
                    className="w-full p-3.5 sm:p-4.5 rounded-xl sm:rounded-2xl bg-[#111111] hover:bg-white/[0.06] border border-white/5 hover:border-[#d49a55]/40 text-left transition-all duration-300 group cursor-pointer"
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <h4 className="text-xs sm:text-sm font-semibold text-[#f7eedf] group-hover:text-white transition-colors">
                          {option.text}
                        </h4>
                        <p className="text-[11px] sm:text-xs text-[#8c8276] mt-0.5 sm:mt-1 font-normal leading-relaxed">
                          {option.description}
                        </p>
                      </div>
                      <ArrowRight className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#5e554a] group-hover:text-[#d49a55] shrink-0 mt-0.5 transition-colors" />
                    </div>
                  </button>
                ))}
              </div>
            ...
            <div className="space-y-4 sm:space-y-6 animate-in zoom-in-95 duration-400">
              ...
              <div className="text-center space-y-1.5">
                <span className="text-[10px] uppercase tracking-[0.25em] text-[#d49a55] font-semibold">
                  Tu Arquetipo de Café LAVA
                </span>
                <h3 className="text-xl sm:text-3xl font-bold text-[#f7eedf]">
              ...
              <div className="p-4 sm:p-6 rounded-2xl bg-[#111111] border border-white/10 space-y-3 sm:space-y-4">
                <div className="flex items-center justify-between border-b border-white/5 pb-3">
                  ...
                  <span className="px-2.5 py-0.5 sm:py-1 rounded-full text-[11px] sm:text-xs font-semibold bg-[#d49a55]/10 border border-[#d49a55]/30 text-[#d49a55]">
                    {resultBean.altitude}
                  </span>
                </div>
                ...
                {/* Ritual ideal */}
                <div className="p-2.5 sm:p-3 rounded-xl bg-white/[0.02] border border-white/5 text-xs text-[#d6c9b8] flex items-center gap-2">
              ...
              <div className="space-y-2.5 sm:space-y-3 pt-1">
                <button
                  ...
                  className="w-full py-3.5 sm:py-4 px-4 sm:px-6 rounded-xl sm:rounded-2xl bg-[#25D366]/20 hover:bg-[#25D366] border border-[#25D366]/50 text-[#25D366] hover:text-black font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-2 transition-all cursor-pointer shadow-lg"
                >
                ...
                <div className="grid grid-cols-2 gap-2.5 sm:gap-3">
                  <button
                    ...
                    className="py-2.5 sm:py-3 px-2 text-center rounded-xl border border-white/10 text-xs font-semibold text-[#d6c9b8] hover:text-white hover:border-[#d49a55]/40 transition-all cursor-pointer bg-white/[0.02]"
                  >
                    Personalizar Molienda
                  </button>

                  <button
                    onClick={handleReset}
                    className="py-2.5 sm:py-3 rounded-xl border border-white/5 text-xs font-medium text-[#7d7367] hover:text-white transition-all cursor-pointer flex items-center justify-center gap-1.5"
                  >
